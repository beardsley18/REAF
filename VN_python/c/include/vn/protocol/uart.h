...
* \param[in] length The number of bytes in the data packet. */
...
typedef struct
{
	...
	*   stream where packets are found.	*/
	...
	char receiveBuffer[VNUART_PROTOCOL_BUFFER_SIZE];

} VnUartPacketFinder;

/** \brief Initializes the VnUartPacketFinder data structure.
 *
 * \param[in] toInitialize The VnUartPacketFinder structure to initialize.
 */
void VnUartPacketFinder_initialize(VnUartPacketFinder* toInitialize);

/** \brief Processes data received from the UART.
 *
 * \param[in] finder The associated VnUartPacketFinder containing the data
 *     processing state.
 * \param[in] data The received data.
 * \param[in] length The number of bytes received.
 */
...
/** \brief Converts a VnAsciiAsync enum into a string.
*
* \param[out] out The buffer to place the string in.
* \param[in] val The VnAsciiAsync enum value to convert to string.
