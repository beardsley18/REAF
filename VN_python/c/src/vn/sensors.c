...
VnError VnSensor_changeBuadrate(VnSensor* s, uint32_t baudrate)
{
	VnError error;
...
VnError VnSensor_transaction(VnSensor* s, char* toSend, size_t toSendLength, char* response, size_t* responseLength)
{
	...
	{
		VnUartPacket_finalizeCommand(s->sendErrorDetectionMode, buffer, &finalLength);
	}
...
void VnSensor_dataReceivedHandler(void *userData)
{
	...
	VnUartPacketFinder_processReceivedData(&s->packetFinder, readBuffer, numOfBytesRead);
	...
	while (true)
	{
		...
		if (waitResult == E_SIGNALED)
		{
			...
			*responseLength = s->responseLength;
			...
			if (VnUartPacket_isErrorRaw(responseBuffer))
			{
				uint8_t sensorError;

				s->waitingForResponse = false;
				VnUartPacket_parseErrorRaw(responseBuffer, &sensorError);
...
VnError VnSensor_readBinaryOutput1(VnSensor *s, BinaryOutputRegister *fields)
{
	...
	if ((error = VnUartPacket_genReadBinaryOutput1(toSend, sizeof(toSend), s->sendErrorDetectionMode, &length)) != E_NONE)
		return error;
...
VnError VnSensor_writeBinaryOutput1(VnSensor *s, BinaryOutputRegister *fields, bool waitForReply)
{
	...
	if ((error = VnUartPacket_genWriteBinaryOutput1(toSend, sizeof(toSend), s->sendErrorDetectionMode, &length, fields->asyncMode, fields->rateDivisor, fields->commonField, fields->timeField, fields->imuField, fields->gpsField, fields->attitudeField, fields->insField)) != E_NONE)
		return error;
...
VnError VnSensor_readBinaryOutput2(VnSensor *s, BinaryOutputRegister *fields)
{
	...
	if ((error = VnUartPacket_genReadBinaryOutput2(toSend, sizeof(toSend), s->sendErrorDetectionMode, &length)) != E_NONE)
		return error;
...
VnError VnSensor_writeBinaryOutput2(VnSensor *s, BinaryOutputRegister *fields, bool waitForReply)
{
	...
	if ((error = VnUartPacket_genWriteBinaryOutput2(toSend, sizeof(toSend), s->sendErrorDetectionMode, &length, fields->asyncMode, fields->rateDivisor, fields->commonField, fields->timeField, fields->imuField, fields->gpsField, fields->attitudeField, fields->insField)) != E_NONE)
		return error;
...
VnError VnSensor_readBinaryOutput3(VnSensor *s, BinaryOutputRegister *fields)
{
	...
	if ((error = VnUartPacket_genReadBinaryOutput3(toSend, sizeof(toSend), s->sendErrorDetectionMode, &length)) != E_NONE)
		return error;
...
VnError VnSensor_writeBinaryOutput3(VnSensor *s, BinaryOutputRegister *fields, bool waitForReply)
{
	...
	if ((error = VnUartPacket_genWriteBinaryOutput3(toSend, sizeof(toSend), s->sendErrorDetectionMode, &length, fields->asyncMode, fields->rateDivisor, fields->commonField, fields->timeField, fields->imuField, fields->gpsField, fields->attitudeField, fields->insField)) != E_NONE)
		return error;
