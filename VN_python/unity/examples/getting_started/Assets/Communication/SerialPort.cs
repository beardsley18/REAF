...
public class SerialPort : ISimplePort, IDisposable
{
	...
	private void CloseAfterUsbCableUnplugged()
	{
		...
		// TODO: Need to implement Linux version.
	...
	{
		_singleByteReadIn = (byte) _sp.ReadByte();
	...
	private void HandlerWhileRaw(ref bool flagIfUserUnpluggedUsb)
	{
		Linux.FD_SET(_handle, ref readFs);
		...
		if (!Linux.FD_ISSET(_handle, ref readFs))
			return false;
